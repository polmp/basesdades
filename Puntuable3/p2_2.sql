...
CREATE TRIGGER IF NOT EXISTS modificar_sou BEFORE UPDATE ON empleat 
FOR EACH ROW 
	WHEN (new.sou != old.sou) 
BEGIN
	SELECT RAISE(ABORT,'No hi ha suficients usuaris o el sou es massa baix!') 
		WHERE (
				(SELECT count(*) from EMPLEAT) 
					< 200) 
			or (new.sou < 100);
END;
