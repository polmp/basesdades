...
CREATE TRIGGER IF NOT EXISTS canvia_grau_null AFTER INSERT ON USUARIS 
	WHEN NEW.grau < 9 or NEW.grau > 12 
BEGIN
	UPDATE usuaris set grau = null WHERE id = NEW.id;
END;

CREATE TRIGGER IF NOT EXISTS canvia_grau_9 AFTER INSERT ON USUARIS 
...
CREATE TRIGGER IF NOT EXISTS afegeix_simetria AFTER INSERT ON amistats 
BEGIN
	INSERT OR IGNORE INTO amistats values (NEW.ID2,NEW.ID1);
END;

CREATE TRIGGER IF NOT EXISTS borra_simetria DELETE ON amistats 
BEGIN
...
CREATE TRIGGER IF NOT EXISTS borra_estudiant UPDATE ON usuaris 
	WHEN NEW.grau > 12 
BEGIN
...
CREATE TRIGGER IF NOT EXISTS incrementa_grau AFTER UPDATE ON usuaris 
	WHEN NEW.grau = OLD.grau+1 
BEGIN
	UPDATE usuaris set grau=grau+1 
		WHERE ID IN 
			(SELECT ID1 from amistats where ID2 = NEW.ID);
END;
...
CREATE TRIGGER IF NOT EXISTS borra_amics_preferencia BEFORE UPDATE ON preferencies 
for each row
BEGIN
	--DELETE FROM amistats where (ID1=OLD.ID2 and ID2=NEW.ID2) OR (ID1=NEW.ID2 and ID2=OLD.ID2);
	delete from amistats where ID1=(select new.ID2_2 from preferencies where old.ID2_2<>new.ID2_2) and 
		ID2=(select old.ID2_2 from preferencies where old.ID2_2<>new.ID2_2);
END;
